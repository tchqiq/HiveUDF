 ...
 */
@Description(
        name = "extract_toUser_from_sta",
        value = "_FUNC_(str) - 从edm日志中抽取toUser",
        extended = "Example:\n" +
                "  > SELECT extract_toUser_from_sta(sta) FROM file_test a;\n"
)
public final class ExtractEdmToUser extends UDF {
	public final static String regexpStr = "(to=<)([\\w!#$%&'*+/=?^_`{|}~-]+(?:\\.[\\w!#$%&'*+/=?^_`{|}~-]+)*@(?:[\\w](?:[\\w-]*[\\w])?\\.)+[\\w](?:[\\w-]*[\\w])?)";
	private Text r =  new Text();
    public static void main(String[] args) {
    	ExtractEdmToUser ep = new ExtractEdmToUser();
		Text t = new Text();
