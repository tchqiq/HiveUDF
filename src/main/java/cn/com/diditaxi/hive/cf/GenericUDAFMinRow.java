...
@Description(name = "minrow", 
			 value = "_FUNC_(expr) " +
			 		"- Returns the minmum value of expr and values of associated columns as a struct")
public class GenericUDAFMinRow extends AbstractGenericUDAFResolver {

	static final Log LOG = LogFactory.getLog(GenericUDAFMinRow.class.getName());
	...
	public static class GenericUDAFMaxRowEvaluator extends GenericUDAFEvaluator {
		...
		private void initMapSide(ObjectInspector[] parameters)
			...
			for (int i = 0; i < length; i++) {
				fieldNames.add("col" + i); // field names are not made
											// available! :(
				outputOIs[i] = ObjectInspectorUtils
						.getStandardObjectInspector(parameters[i]);
			}

			inputOIs = parameters;
			structOI = ObjectInspectorFactory.getStandardStructObjectInspector(
					fieldNames, fieldOIs);
		}

		/*
		 * Initialize the UDAF on the reduce side (or the map side in some
		 * cases).
		 */
		private void initReduceSide(StructObjectInspector inputStructOI)
				throws HiveException {
			List<? extends StructField> fields = inputStructOI
			...
			for (int i = 0; i < length; i++) {
				...
				outputOIs[i] = ObjectInspectorUtils
						.getStandardObjectInspector(inputOIs[i]);
			}
			structOI = ObjectInspectorUtils
					.getStandardObjectInspector(inputStructOI);
		}
		...
		public void merge(AggregationBuffer agg, Object partial)
			...
			if (partial != null) {
				...
				if (partial instanceof Object[]) {
					...
					throw new HiveException("Invalid type: "
				...
				if (maxagg.objects == null) {
					...
					int cmp = ObjectInspectorUtils.compare(maxagg.objects[0],
							outputOIs[0], objects.get(0), inputOIs[0]);
					if (cmp > 0) {
						isMin = true;
					}
				}
				...
				if (isMin) {
					...
					for (int i = 0; i < length; i++) {
						maxagg.objects[i] = ObjectInspectorUtils
								.copyToStandardObject(objects.get(i),
										inputOIs[i]);
					}
				}
			}
